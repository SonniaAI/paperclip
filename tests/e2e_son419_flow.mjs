...
function runPython(databaseUrl) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.env.PYTHON ?? 'python3', ['tests/e2e_son419_flow.py'], {
      cwd: join(here, '..'),
      env: {
        ...process.env,
    ...
    child.on('exit', (code, signal) => {
      if (code === 0) resolve()
      else reject(new Error(`SON-419 e2e runner exited with code ${code ?? 'null'} (${signal ?? 'no signal'})`))
    })
  })
}
...
try {
  ...
  console.log('PASS: SON-419 routes verified end to end over PostgreSQL wire protocol.')
} finally {
  await server.stop()
  await db.close()
}
